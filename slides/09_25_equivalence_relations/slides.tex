...
\small
\only<9>{\footnotesize}
\colorbox{yellow!30}{\textbf{Goal.}} Let's state the three main properties that relations \alert{might} have and try to give an example/non-example for each.
\pause 
...
\begin{enumerate}
	...
	\vspace{-.2cm} 
	\item \only<6,9>{\textbf{Symmetry.} \pause The \texttt{friends-of} relation is symmetric (usually -- \includegraphics[height=1em]{images/eek}), but \texttt{parents-of} relation is not.}
